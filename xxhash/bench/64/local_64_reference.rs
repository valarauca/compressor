...
extern crate xxhash;
use xxhash::bits64::xxhash64_reference;

fn bench_local_64_reference(c: &mut Criterion) {
    let mut data = [0u8; 32786];
    getrandom(data.as_mut()).unwrap();
    let mut group = c.benchmark_group("xxhash64_internal_reference_size");
    for size in vec![1, 4, 8, 16, 32usize] {
        ...
        group.bench_with_input(BenchmarkId::from_parameter(sized), &slice, |b, data| {
            b.iter(|| {
                let _ = black_box(xxhash64_reference(0, data));
            });
        });
    }
}
criterion_group!(benches, bench_local_64_reference);
